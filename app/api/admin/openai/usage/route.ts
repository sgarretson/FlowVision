...
export async function GET() {
  try {
    ...
    const lastMonthUsage = aiLogs.filter(
      (log) => log.timestamp >= lastMonth && log.timestamp < thisMonth
    );

    // Group by action type
    const usageByType = aiLogs.reduce(
      (acc, log) => {
    ...
    // Group by user
    const usageByUser = aiLogs.reduce(
      (acc, log) => {
    ...
    const stats = {
      ...
      monthlyGrowth:
        lastMonthUsage.length > 0
          ? (
              ((thisMonthUsage.length - lastMonthUsage.length) / lastMonthUsage.length) *
              100
            ).toFixed(1)
