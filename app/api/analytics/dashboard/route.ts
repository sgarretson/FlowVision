...
export async function GET(request: NextRequest) {
  try {
    ...
    const phaseDistribution = initiativesByPhase.reduce((acc, item) => {
    ...
    const avgCompletionTime = completedInitiativesWithDates.length > 0
      ? completedInitiativesWithDates.reduce((sum, init) => {
          const days = Math.ceil((new Date(init.updatedAt).getTime() - new Date(init.createdAt).getTime()) / (1000 * 60 * 60 * 24));
          return sum + days;
