...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: 'Failed to generate ROI forecast' }, { status: 500 });
  }
}
...
async function calculateCurrentRoi(initiatives: any[]) {
  ...
  totalInvestment = initiatives.filter((i) => i.budget).reduce((sum, i) => sum + i.budget, 0);
...
function calculateTrendMultiplier(initiatives: any[]): number {
  ...
  const recentInitiatives = initiatives.filter((i) => new Date(i.createdAt) >= thirtyDaysAgo);

  const recentUpdates = initiatives.filter(
    (i) => new Date(i.updatedAt) >= thirtyDaysAgo && new Date(i.updatedAt) > new Date(i.createdAt)
  );
...
function generateRoiRecommendations(initiatives: any[], current: any, forecast: any): string[] {
  ...
  if (current.realizedRoi < 10) {
    recommendations.push(
      'Consider reviewing initiative selection criteria to improve ROI outcomes'
    );
  } else if (current.realizedRoi > 25) {
    recommendations.push(
      'Excellent ROI performance - consider scaling successful initiative patterns'
    );
  }
  ...
  if (activeCount > completedCount * 2) {
    recommendations.push(
      'High number of active initiatives - consider focusing resources for faster completion'
    );
  }
  ...
  if (forecast.confidence < 60) {
    recommendations.push(
      'Low forecast confidence - gather more initiative outcome data to improve predictions'
    );
  }
  ...
  if (current.totalInvestment < 50000) {
    recommendations.push(
      'Consider increasing initiative investment to drive more significant organizational impact'
    );
  }
  ...
  if (overdueInitiatives.length > 0) {
    recommendations.push(
      `${overdueInitiatives.length} initiatives are overdue - review resource allocation and priorities`
    );
  }
  ...
  if (recommendations.length === 0) {
    recommendations.push(
      'Initiative portfolio is performing well - maintain current execution standards'
    );
  }
