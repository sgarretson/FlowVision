...
export async function GET(request: NextRequest) {
  try {
    ...
    const healthScore = Math.round(
      initiativeHealth * 0.3 + issueVelocity * 0.25 + teamUtilization * 0.2 + roiTrend * 0.25
    );
...
function calculateInitiativeHealth(initiatives: any[]): number {
  ...
  const recentlyUpdated = initiatives.filter((init) => {
    const daysSinceUpdate =
      (now.getTime() - new Date(init.updatedAt).getTime()) / (1000 * 60 * 60 * 24);
...
function calculateIssueVelocity(issues: any[]): number {
  ...
  const averageHeatmap =
    issues.reduce((sum, issue) => sum + (issue.heatmapScore || 50), 0) / issues.length;
