...
export async function POST(request: NextRequest) {
  try {
      ...
      for (const otherIssue of issues) {
        if (issue.id !== otherIssue.id) {
          ...
          if (similarity >= threshold) {
            const commonKeywords = issue.keywords?.filter(keyword => 
              otherIssue.keywords?.includes(keyword)
            ) || [];
    ...
    const clusteringInsights = {
      ...
      averageSimilarity: similarities.reduce((sum, s) => 
        sum + (s.similarIssues[0]?.similarity || 0), 0) / similarities.length,
      strongClusters: similarities.filter(s => 
        s.suggestedCluster && s.suggestedCluster.confidence > 0.7).length,
      recommendations: {
        newClustersNeeded: similarities.filter(s => !s.suggestedCluster).length,
        clusterRefinements: similarities.filter(s => 
          s.suggestedCluster && s.suggestedCluster.confidence < 0.5).length
      }
    };

    // Update cluster metrics
    for (const cluster of clusters) {
      const clusterIssues = similarities.filter(s => 
        s.suggestedCluster?.clusterId === cluster.id
      );
      
      if (clusterIssues.length > 0) {
        const avgConfidence = clusterIssues.reduce((sum, issue) => 
          sum + (issue.suggestedCluster?.confidence || 0), 0) / clusterIssues.length;
