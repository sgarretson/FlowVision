...
async function populateIssueClusters() {
  ...
  const clusters = [
    {
      ...
      description: 'Issues related to coordination, communication, and collaboration across teams. Addresses municipal approvals, team coordination, access management, and cross-departmental workflow challenges.',
      ...
      rootCauses: {
        ...
        secondary: ['Manual approval processes', 'Fragmented tool access', 'Inconsistent review cycles'],
        systemicIssues: ['No unified project visibility', 'Department silos', 'External dependency unpredictability']
      },
      impactAnalysis: {
    ...
    },
    {
      ...
      description: 'Issues related to software, tools, technology integration, and digital workflows. Addresses CAD/BIM compatibility, version control, and cross-platform file sharing challenges.',
      category: 'technology',
      severity: 'high',
      ...
      rootCauses: {
        ...
        secondary: ['Manual version control', 'Incompatible file formats', 'Department-specific tool preferences'],
        systemicIssues: ['No enterprise architecture strategy', 'Legacy system dependencies', 'Insufficient IT coordination']
      },
      impactAnalysis: {
    ...
    },
    {
      ...
      description: 'Issues related to standardization, quality control, and process improvement. Addresses compliance verification, design standards, and systematic process implementation.',
      category: 'process',
      severity: 'medium',
      ...
      rootCauses: {
        ...
        secondary: ['Scattered documentation', 'No automated compliance checking', 'Inconsistent quality standards'],
        systemicIssues: ['Knowledge silos', 'Lack of process automation', 'No standardized methodologies']
      },
      impactAnalysis: {
    ...
    },
    {
      ...
      description: 'Issues related to resource allocation, project management, and operational efficiency. Addresses resource optimization, project tracking, budget management, and strategic planning.',
      category: 'management',
      severity: 'high',
      ...
      rootCauses: {
        ...
        secondary: ['Spreadsheet-based tracking', 'No predictive allocation', 'Post-project financial analysis'],
        systemicIssues: ['Lack of integrated project management', 'No resource optimization algorithms', 'Reactive vs. proactive management']
      },
      impactAnalysis: {
    ...
    },
    {
      ...
      description: 'Issues related to knowledge management, documentation, and information accessibility. Addresses institutional knowledge preservation, material management, and information findability.',
      category: 'knowledge',
      severity: 'medium',
      ...
      rootCauses: {
        ...
        secondary: ['No centralized documentation', 'Manual portfolio updates', 'Knowledge exit risk'],
        systemicIssues: ['No knowledge capture processes', 'Fragmented information systems', 'Institutional memory dependency']
      },
      impactAnalysis: {
    ...
    const cluster = await prisma.issueCluster.create({
      data: {
        ...
        metrics: {
          totalIssues: 0,
          averageSeverity: 0,
          estimatedROI: clusterData.category === 'coordination' ? '25-35%' :
                       clusterData.category === 'technology' ? '20-30%' :
                       clusterData.category === 'process' ? '15-25%' :
                       clusterData.category === 'management' ? '20-40%' : '10-20%',
  ...
  for (const issue of issues) {
    ...
    if (matchingMapping) {
      ...
      const crossImpact = {
        departmentsAffected: description.includes('multiple') || description.includes('across') ? 3 : 1,
        severity: issue.heatmapScore > 85 ? 'high' : issue.heatmapScore > 70 ? 'medium' : 'low',
        coordinationRequired: description.includes('coordination') || description.includes('collaboration'),
      ...
      clusteredCount++;
      console.log(`✅ Clustered: "${issue.description.substring(0, 60)}..." → ${createdClusters.find(c => c.id === matchingMapping.clusterId)?.name}`);
    }
  }
  ...
  for (const cluster of createdClusters) {
    ...
    const averageScore = clusterIssues.length > 0 
      ? Math.round(clusterIssues.reduce((sum, issue) => sum + issue.heatmapScore, 0) / clusterIssues.length)
      : 0;
    ...
    console.log(`📊 Updated metrics for "${cluster.name}": ${clusterIssues.length} issues, avg score ${averageScore}`);
  }
  ...
  console.log(`Successfully Clustered: ${clusteredCount} (${Math.round(clusteredCount/issues.length*100)}%)`);
