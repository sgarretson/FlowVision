  ...
  try {
    ...
    clusters.forEach((cluster, index) => {
      const analytics = {
        ...
        avgInitiativeProgress: cluster.initiatives.length > 0
      ...
      console.log(`      Score Distribution: High(${analytics.scoreDistribution.high}) Med(${analytics.scoreDistribution.medium}) Low(${analytics.scoreDistribution.low})`);
      console.log(`      Initiatives: ${analytics.initiativeCount} (avg progress: ${analytics.avgInitiativeProgress}%)`);
    ...
    Object.entries(departmentBreakdown).forEach(([dept, issueList]) => {
      const avgScore = Math.round(issueList.reduce((sum, issue) => sum + issue.heatmapScore, 0) / issueList.length);
      console.log(`   ${dept}: ${issueList.length} issues (avg score: ${avgScore})`);
    });

    // Test cross-impact issues
    const crossImpactIssues = issues.filter(issue => 
    ...
    const strategicRecommendations = clusters.map(cluster => {
      ...
      if (issues.length >= 3) {
        const avgScore = Math.round(issues.reduce((sum, issue) => sum + issue.heatmapScore, 0) / issues.length);
        if (avgScore > 80) {
          recommendations.push({
            ...
            estimatedROI: cluster.category === 'coordination' ? '25-35%' : 
                         cluster.category === 'technology' ? '20-30%' : '15-25%'
          });
        }
      }
    ...
    const mockClusteringResponse = {
      ...
      insights: {
        ...
        averageSimilarity: similarities.length > 0 
          ? Math.round(similarities.reduce((sum, s) => sum + s.similarity, 0) / similarities.length * 100) / 100
          : 0,
