...
export default function ResourceView({ teamUtilization, initiatives, onTeamClick }: ResourceViewProps) {
  ...
  const averageUtilization = totalCapacity > 0 ? Math.round((totalAllocated / (totalCapacity * 4)) * 100) : 0;
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center space-x-6">
                      ...
                      <div className="text-right min-w-[80px]">
                        <div className={`text-lg font-bold ${getUtilizationTextColor(team.utilizationPercent)}`}>
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-200">
                      ...
                      <div className="space-y-2">
                        {team.assignments.map((assignment) => (
                          <div key={assignment.id} className="flex items-center justify-between py-2 px-3 bg-gray-50 rounded-lg">
                            <div className="flex items-center space-x-3">
                              <span className={`px-2 py-1 text-xs font-medium rounded-full ${getStatusBadgeColor(assignment.initiative.status)}`}>
                                {assignment.initiative.status}
                              </span>
                              <span className="font-medium text-gray-900">{assignment.initiative.title}</span>
                              <span className="text-sm text-gray-600">({assignment.role})</span>
                            </div>
                            <div className="flex items-center space-x-4 text-sm text-gray-600">
                              {assignment.initiative.timelineStart && assignment.initiative.timelineEnd && (
                                <span>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {initiatives.map((initiative) => (
                    <tr key={initiative.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2 py-1 text-xs font-medium rounded-full ${getStatusBadgeColor(initiative.status)}`}>
                          {initiative.status}
                        </span>
                      </td>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {initiative.assignments.map((assignment, index) => (
                            <div key={index} className="flex items-center space-x-1 bg-gray-100 rounded-full px-3 py-1">
                              <span className="text-sm font-medium text-gray-900">{assignment.team.name}</span>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm font-medium text-gray-900">
                          {initiative.assignments.reduce((acc, assignment) => acc + assignment.hoursAllocated, 0)}h
                        </div>
                      </td>
                    </tr>
                  ))}
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              ...
              <p className="mt-1 text-sm text-red-700">
                ...
                {overallocatedTeams.length === 1 ? ' is' : ' are'} overallocated. 
                Consider redistributing workload or adjusting timelines.
              </p>
              <div className="mt-2">
                <div className="flex flex-wrap gap-2">
                  {overallocatedTeams.map((team) => (
                    <span key={team.id} className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800">
