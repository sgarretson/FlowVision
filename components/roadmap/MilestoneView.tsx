...
export default function MilestoneView({ initiatives, onMilestoneClick }: MilestoneViewProps) {
  ...
  const overdueMilestones = allMilestones.filter(m => 
    m.status !== 'completed' && isBefore(new Date(m.dueDate), new Date())
  ).length;
          ...
          {sortedMilestones.length === 0 ? (
            ...
            <div className="space-y-4">
              {sortedMilestones.map((milestone) => {
                const isOverdue = milestone.status !== 'completed' && isBefore(new Date(milestone.dueDate), new Date());
                const isDueToday = isToday(new Date(milestone.dueDate));
                const dueSoon = !isOverdue && !isDueToday && isBefore(new Date(milestone.dueDate), new Date(Date.now() + 7 * 24 * 60 * 60 * 1000));
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex items-start space-x-3 flex-1">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center space-x-4 text-sm text-gray-500">
                            <span className="font-medium text-gray-700">{milestone.initiative.title}</span>
                            <span>•</span>
                      ...
                      <div className="flex items-center space-x-4 flex-shrink-0">
                        ...
                        <div className="text-right">
                          ...
                          <span className={`px-2 py-1 text-xs font-medium rounded-full ${getStatusColor(milestone.initiative.status)}`}>
