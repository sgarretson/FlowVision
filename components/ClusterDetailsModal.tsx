...
export default function ClusterDetailsModal({ clusterId, onClose }: ClusterDetailsModalProps) {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'issues' | 'initiatives' | 'analytics'>('overview');
  ...
  function getTypeIcon(type: string) {
    switch (type.toLowerCase()) {
      case 'process improvement': return '🔄';
              ...
              <>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-4 gap-4">
                      <div className="bg-gray-50 p-4 rounded-lg text-center">
                        <div className="text-2xl font-bold text-blue-600">{cluster.analytics.totalIssues}</div>
                        <div className="text-sm text-gray-600">Total Issues</div>
                      </div>
                      <div className="bg-gray-50 p-4 rounded-lg text-center">
                        <div className="text-2xl font-bold text-green-600">{cluster.analytics.averageScore}</div>
                        <div className="text-sm text-gray-600">Avg Score</div>
                      </div>
                      <div className="bg-gray-50 p-4 rounded-lg text-center">
                        <div className="text-2xl font-bold text-purple-600">{cluster.analytics.initiativeProgress.total}</div>
                        <div className="text-sm text-gray-600">Initiatives</div>
                      </div>
                      <div className="bg-gray-50 p-4 rounded-lg text-center">
                        <div className="text-2xl font-bold text-orange-600">{cluster.analytics.totalVotes}</div>
                    ...
                    {cluster.recommendations.length > 0 && (
                      <div>
                        <h3 className="text-lg font-medium text-gray-900 mb-3">Strategic Recommendations</h3>
                        ...
                        <div key={issue.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                          <div className="flex justify-between items-start mb-3">
                            ...
                            <div className="flex items-center space-x-3">
                              ...
                              <span className="text-lg font-semibold text-blue-600">{issue.heatmapScore}</span>
                            </div>
                          </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-4">
                      {cluster.initiatives.map((initiative) => (
                        <div key={initiative.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                          <div className="flex justify-between items-start mb-3">
                            <div>
                            ...
                            <div className="text-right">
                              <div className="text-lg font-semibold text-green-600">{initiative.progress}%</div>
                          ...
                          {initiative.addressedIssues.length > 0 && (
                            <div className="mb-3">
                              <div className="text-xs text-gray-500 mb-1">Addresses {initiative.addressedIssues.length} issues:</div>
                              <div className="flex flex-wrap gap-1">
                                {initiative.addressedIssues.slice(0, 3).map((issue) => (
                                  <span key={issue.id} className="inline-flex items-center px-2 py-0.5 rounded text-xs bg-gray-100 text-gray-700">
                                    Score: {issue.heatmapScore}
                                  </span>
                                ))}
                                {initiative.addressedIssues.length > 3 && (
                                  <span className="text-xs text-gray-500">+{initiative.addressedIssues.length - 3} more</span>
                                )}
                              </div>
                            </div>
                          )}
                ...
                {activeTab === 'analytics' && (
                  <div className="space-y-6">
                    {/* Score Distribution */}
                    <div>
                      <h3 className="text-lg font-medium text-gray-900 mb-4">Issue Score Distribution</h3>
                      <div className="grid grid-cols-3 gap-4">
                        <div className="bg-red-50 p-4 rounded-lg text-center">
                          <div className="text-2xl font-bold text-red-600">{cluster.analytics.scoreDistribution.high}</div>
                          <div className="text-sm text-red-700">High Priority (85+)</div>
                        </div>
                        <div className="bg-yellow-50 p-4 rounded-lg text-center">
                          <div className="text-2xl font-bold text-yellow-600">{cluster.analytics.scoreDistribution.medium}</div>
                          <div className="text-sm text-yellow-700">Medium Priority (70-85)</div>
                        </div>
                        <div className="bg-green-50 p-4 rounded-lg text-center">
                          <div className="text-2xl font-bold text-green-600">{cluster.analytics.scoreDistribution.low}</div>
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        {Object.entries(cluster.analytics.departmentBreakdown).map(([dept, count]) => (
                    ...
                    {/* Initiative Progress */}
                    <div>
                      <h3 className="text-lg font-medium text-gray-900 mb-4">Initiative Progress</h3>
                      <div className="grid grid-cols-2 gap-4">
                        <div className="bg-blue-50 p-4 rounded-lg">
                          <div className="text-lg font-semibold text-blue-600">{cluster.analytics.initiativeProgress.active}</div>
                          <div className="text-sm text-blue-700">Active Initiatives</div>
                        </div>
                        <div className="bg-green-50 p-4 rounded-lg">
                          <div className="text-lg font-semibold text-green-600">{cluster.analytics.initiativeProgress.completed}</div>
                      ...
                      {cluster.analytics.initiativeProgress.total > 0 && (
                        <div className="mt-4 p-4 bg-gray-50 rounded-lg">
                          <div className="flex items-center justify-between mb-2">
                            ...
                            <span className="text-sm font-semibold text-gray-900">{cluster.analytics.initiativeProgress.averageProgress}%</span>
                          </div>
                          <div className="bg-gray-200 rounded-full h-2">
                            <div 
                              className="bg-blue-600 h-2 rounded-full transition-all duration-300"
                              style={{ width: `${cluster.analytics.initiativeProgress.averageProgress}%` }}
                            ></div>
                          </div>
                        </div>
                      )}
